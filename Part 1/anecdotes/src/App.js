...
const Display = (props) => {
	// When no vote is given
	if(props.type === "Maximum" && props.votes === 0){
		return (
			<>No votes given yet</>
		)
	}
...
const App = () => {
	...
	const handleVote = () => {
		// Copy vote, update and change state
		let copyVote = [...vote]
		copyVote[selected]++
		setVote(copyVote)
	}
	...
	return (
		<div>
			...
			<Display anecdote={anecdotes[maxVote]} votes={vote[maxVote]} type="Maximum"/>
		</div>
	)
}
